...
public class DetectRomance {
				...
				for (String elem : data) {
					if (line.contains("Microsoft-Windows-Security-Auditing,")) {
						...
						if (elem.contains("アカウント名:")
							...
							try {
								// Get date
								if (shredName.endsWith(SHARE_NAME) || shredName.endsWith(SHARE_NAME_ADMIN)) {
			...
			for (EventLogData ev : evS) {
				if (5140 == ev.getEventID()) {
					if ((ev.getAccountName().endsWith("$") && ev.getSharedName().contains(SHARE_NAME))
						...
						//System.out.println(ev.getSharedName()+","+ev.getAccountName());
					} 
	...
	private void outputLogs(Map<Long, LinkedHashSet> kerlog) {
		for (Iterator it = kerlog.entrySet().iterator(); it.hasNext();) {
			...
			for (EventLogData ev : evS) {
				...
				if (
						//eventID == EVENT_PRIV_OPE || eventID == EVENT_PROCESS
						eventID == EVENT_SHARE) {
	...
	public static void main(String args[]) throws ParseException {
		DetectRomance authLogParser = new DetectRomance();
		String inputdirname = "";
		if (args.length < 2) {
			printUseage();
		} else
